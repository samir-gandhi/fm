...
while true; do
    ### OVERWRITTEN FOR HTTP
    status=$(curl -H "x-user-id: ${PAP_ADMIN}" -k http://localhost:${HTTP_PORT}/api/system/status 2>/dev/null | jq -r '.status')
...
if test -d "${STAGING_DIR}/policies" ; then
    for policyFileName in $( find "${STAGING_DIR}/policies/" -iname \*.SNAPSHOT 2>/dev/null | sort | uniq ) ; do
        ...
        ### OVERWRITTEN FOR HTTP
        snapshotId=$(curl -H "x-user-id: ${PAP_ADMIN}" -k -d @${policyFileName} http://localhost:${HTTP_PORT}/api/snapshot/${policyName}/import | jq -r '.id')
